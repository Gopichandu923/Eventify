    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh] px-4">
        <div className="text-center glass-card p-10 rounded-3xl border-rose-500/20 max-w-sm w-full">
          ...
          <Link href="/organizer/auth" className="block w-full py-4 bg-white text-indigo-600 font-black text-[10px] rounded-xl hover:bg-neutral-100 transition-all uppercase tracking-widest shadow-xl shadow-white/5">
            Re-Authenticate
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="max-w-6xl mx-auto pb-10">
      <div className="glass-card rounded-[2rem] md:rounded-[2.5rem] overflow-hidden mb-6 border-white/5 mx-4 md:mx-0">
        <div className="bg-gradient-to-br from-indigo-600 via-indigo-900 to-slate-900 p-5 md:p-10 relative overflow-hidden text-center md:text-left">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between relative z-10 gap-5">
            ...
            <button onClick={handleLogout} className="px-4 py-2.5 bg-white/5 hover:bg-white/10 text-white font-black rounded-lg border border-white/10 transition-all flex items-center justify-center text-[9px] uppercase tracking-widest self-center md:self-auto">
      ...
      <div className="flex flex-col md:flex-row gap-3 mb-6 px-4 md:px-0">
        ...
        <Link href="/organizer/create-event" className="md:w-48">
      ...
      {eventList.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-6">
          {eventList.map((event) => (
            <div key={event._id} className="group glass-card rounded-[1.5rem] md:rounded-[2.5rem] overflow-hidden hover:border-indigo-500/30 transition-all duration-500 border-white/5 mx-4 md:mx-0">
              <div className="p-5 md:p-8 flex flex-col lg:flex-row lg:items-center justify-between gap-6 md:gap-10">
                  ...
                  <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-4">
                    ...
                    <div className="flex gap-1.5 font-black">
                      ...
                      <span className={`px-2 py-0.5 md:px-3 md:py-1 text-[8px] md:text-[9px] uppercase tracking-widest border rounded-full ${
                        event.approvalMode === "manual" ? "bg-amber-500/10 text-amber-400 border-amber-500/20" : "bg-emerald-500/10 text-emerald-400 border-emerald-500/20"
                      }`}>
                        {event.approvalMode === "manual" ? "Manual" : "Auto"}
                      </span>
                    </div>
                  </div>
                ...
                <div className="flex flex-row md:flex-nowrap gap-3">
                  ...
                  <Link href={`/organizer/registrations/${event._id}`} className="flex-1">
